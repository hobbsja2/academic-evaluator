  ...
  try {
    ...
    return withTransaction(database, async (client) => {
      ...
      const inserted = await client.query(`INSERT INTO rubrics
        (assignment_id, title, version, total_points, source, source_url, captured_at)
        VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING id`,
      [assignments.rows[0].id, rubric.rubricTitle, version, rubric.totalPoints,
        rubric.source, rubric.sourceUrl, rubric.capturedAt]);
...
router.get("/", async (_request, response) => {
  try {
    ...
    const rubrics = await database.query(`SELECT r.id, r.title AS "rubricTitle", r.version,
        r.total_points::float8 AS "totalPoints", r.source, r.source_url AS "sourceUrl",
        r.captured_at::text AS "capturedAt",
